...
function fbmGetSyncSettings() { return runEntryPoint('fbmGetSyncSettings', 'sidebar', 'throw', function () { var spreadsheetId = ''; try { spreadsheetId = String(FbmSync.currentSpreadsheetId ? FbmSync.currentSpreadsheetId() : shinOpenBook().getId() || ''); } catch (ignore) {} return fbmPublicResult({ ok: true, settings: { spreadsheetId: spreadsheetId, accountName: FbmSync.configValue('FBM_ACCOUNT_NAME'), customerPrefix: FbmSync.configValue('FBM_MA_KH_PREFIX'), customerCodeLength: FbmSync.configValue('FBM_MA_KH_LENGTH'), activitySince: FbmSync.configValue('FBM_ACTIVITY_SINCE'), approvalThreshold: FbmSync.approvalThreshold(), extension: typeof FbmSync.extensionConfigPublic === 'function' ? FbmSync.extensionConfigPublic() : { pollMinutes: 5, runOnStartup: true }, background: typeof FbmSync.backgroundSchedulePublic === 'function' ? FbmSync.backgroundSchedulePublic() : {} } }); }); }
