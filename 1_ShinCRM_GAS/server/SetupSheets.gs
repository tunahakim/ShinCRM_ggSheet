...
/**
 * Trả về tệp Sheet để làm việc, kèm cổng chặn tệp sai. Mọi hàm phải lấy tệp qua đây, không gọi thẳng SpreadsheetApp.
 *
 * Có hai đường gọi hàm và chúng khác nhau ở một điểm chí tử. Mở từ menu hoặc bấm Run trong editor thì có "tệp đang mở".
 * Gọi qua Apps Script API để chạy khi không có ai ngồi trước máy thì KHÔNG có tệp đang mở, và
...
function shinOpenBook() {
  ...
  // Cả phép lấy tệp lẫn phép đọc ID đều phải nằm trong try. Chạy qua API thì Google có thể trả về một đối tượng
