...
**BUG QUAN TRỌNG — Escape dấu gạch chéo:** FBM (ASP.NET WebForms) có một bug: nếu JSON body chứa ký tự `/` không được escape, server trả về HTTP 500. GAS phải serialize payload thành chuỗi wire bằng cách thay thế tất cả `/` thành `\/` trước khi tạo envelope gửi Extension. Extension `executor.js` chỉ đưa nguyên văn `bodyText` đó vào `fetch`, không parse, serialize lại, sửa field hay retry request ghi. Extension không được tự quyết định nghiệp vụ.
