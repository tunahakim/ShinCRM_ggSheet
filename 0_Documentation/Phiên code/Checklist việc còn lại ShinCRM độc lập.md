...
- [x] Extension bắn ảnh chụp trạng thái `CRM_CONTEXT` thay tin `CRM_TRIGGER`: gửi sheet/gid, Name Box nguyên bản, hàng 1 thô, `reads` theo địa chỉ, sheetTabs, tín hiệu tương tác, at và seq; không gửi `row/col/selectionKind/cellText/isEditing` như kết luận nghiệp vụ. Hai tệp `2_ShinCRM_Extension/content_scripts/scout/sheet_scout.js` và `.../bridge/iframe_bridge.js`.
...
- [x] Sidebar: tệp mới `client/link/sheetLink.html` kèm dòng include trong `Sidebar.html`; nhận `reads` thô từ `CRM_CONTEXT`, tự đối chiếu giá trị trong Store rồi đi qua `ACTIONS.setCurrentCustomer`, không tra theo dòng và không dựng event bus. `bootstrap.html` gọi `sheetLinkSetSpreadsheetId(core.spreadsheetId)` sau `ingestCore`.
