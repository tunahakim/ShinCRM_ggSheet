...
- [x] Máy chủ: tệp mới `server/service/SelectionService.js` với `probeSelectionCheap` (chỉ tên sheet và ô đang chọn) và `probeSelectionFull` (thêm tra mã khách); thêm `viewProbeSelection` vào danh sách trắng của DevRunner. Sửa `SelectionService.js` dùng `shinOpenBook()` thay `getActiveSpreadsheet()`.
