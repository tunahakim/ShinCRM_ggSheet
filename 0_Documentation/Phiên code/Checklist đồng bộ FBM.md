...
| Slice 9A — Một nhịp Extension + scheduler GAS | `a1b8e0c`, `02331ab`, `3b0ed75`, `14938d6`, `d57cb30` + bản phát hành Extension | `1467/1467` | `@311` | GAS DEV `fbmGetSyncSettings` trả `OK` với `account` từ `FBM_ACCOUNT_SETTINGS_V1`; `fbmSaveAccountSettings` trả `OK` trên Sheet DEV; offline kiểm migration/validation prefix-độ dài/mốc Activity, lọc Activity trước mốc, kiểm mã FBM tự sinh, preflight Customer mới, smoke UI card cấu hình tài khoản và menu bốn màn hình với Chạy đồng bộ là mặc định | Còn cần đo response bulk thật, live kiểm tab/Sidebar và nghiệm thu 44 pipeline theo các mục riêng |
