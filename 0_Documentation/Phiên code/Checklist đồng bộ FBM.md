...
| Slice 8 — UI + log + probe | `e243432`, `3e60e88`, `e4c92c1`, `52c3fa9`, `81c4c58`, `778eed1`, `54056ef`, `4493ec5`, `23d1a49` | `1200/1200` | `@220` (`fbmGetSyncSettings` trả `OK`, `approvalThreshold: 10`; `fbmGetMasterSwitch` trả `OK`, `enabled: true`) | UI shell năm màn hình, menu dọc phù hợp Sidebar hẹp, popup dùng chung cho search/dropdown/customer picker/menu, hộp menu nổi dưới header, pill `ON/OFF` nhỏ, tiêu đề đổi theo màn hình, form liên kết nhập tay, pipeline chỉ hiện khi phiên chạy, tab kết quả và phân trang đã hoàn tất; mở lại dropdown hợp lệ hiện toàn bộ danh sách và bôi xanh giá trị cũ, đang gõ chỉ lọc các dòng khớp; bố cục động dùng Block/schema và renderer chung; chưa nghiệm thu probe trên Sidebar | Cần chủ dự án đóng/mở lại Sidebar để tải deployment mới và kiểm tra hiển thị thực tế |
