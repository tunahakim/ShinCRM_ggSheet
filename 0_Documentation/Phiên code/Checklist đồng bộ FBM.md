...
- [x] Executor có phiên bản giao thức riêng; worker tự nạp lại bản mới và dùng kênh request V2 để bản cũ không xử lý heartbeat song song. Endpoint rỗng, `undefined` hoặc ngoài allowlist bị chặn trước `fetch`, trace chỉ giữ metadata an toàn. Có lệnh `fbmHeartbeatNow()` và `fbmHeartbeatLastStatus` để nghiệm thu ngay, không phải chờ alarm 5 phút; heartbeat `null` được giữ nguyên để executor dùng request Customer mặc định; alarm kiểm tra relay/công tắc trước khi chạm FBM, dừng khi deployment GAS lỗi hoặc phiên hết hạn.
