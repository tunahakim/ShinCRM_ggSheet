...
- [x] Tạo harness `tests/cases/fbmSync/Workflow.js` chạy envelope thật qua GAS -> executor Extension -> GAS với fixture FBM, không dùng request FBM thật; đã kiểm tự điền/kiểm tra liên kết, response thiếu identity, xóa binding rỗng, cổng mode, full read rỗng, alarm noop/no tab/có tab, approval, master OFF, stale response, cancel khi request đang bay, DTO không lộ state nội bộ và log tổng hợp.
