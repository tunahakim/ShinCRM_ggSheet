...
| Slice 9A — Một nhịp Extension + scheduler GAS | `a1b8e0c` + bản phát hành Extension | `1466/1466` | `@309` | GAS DEV `fbmGetSyncSettings`, `fbmSaveExtensionConfig`, `fbmSaveBackgroundSchedule`, `fbmSaveLoginPolicy` đã trả `OK` ở các lần nghiệm thu trước; `fbmGetSyncSettings`, `fbmGetMasterSwitch`, `fbmGetBackgroundSwitch` trả `OK` ở các lần nghiệm thu; `fbmGetSyncSettings` sau thay đổi UI trả `OK` ở `@309`; offline đã kiểm một alarm `gas_poll`, startup/no-op/overlap/ưu tiên/auto-open, continuation nền và projection response lớn | Còn cần đo response bulk thật, live kiểm tab/Sidebar và nghiệm thu 44 pipeline theo các mục riêng |
