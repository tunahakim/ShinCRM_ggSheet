...
- [x] Form thiết lập đặt username và password cạnh nhau, hiển thị username đầy đủ và mật khẩu dạng bản rõ chỉ trong lượt đang gõ để người dùng đối chiếu; không ghi mật khẩu bản rõ vào Sheet hoặc Log. Sidebar chỉ xóa ô sau `Lưu mã hóa` thành công, không xóa sau `Đăng nhập thử`.
- [x] Extension mã hóa username/mã user, SpreadsheetId và mật khẩu thành envelope trước khi gửi/lưu credential qua GAS; không lưu mật khẩu bản rõ ở Sheet, Log hoặc `Config`. Kho cục bộ dùng AES-GCM; metadata username được phép hiển thị đầy đủ.
- [x] Khi đọc lại cấu hình, Extension chỉ giải mã nội bộ và trả trạng thái cùng metadata username/những trường không nhạy cảm; Sidebar không nhận mật khẩu bản rõ. DTO `fbmGetLoginConfig` loại envelope trước khi trả về.
...
### Đợt bổ sung cảnh báo thay đổi chưa lưu Sidebar
...
- [x] Tạo lõi `ActionCoordinator` dùng chung cho mọi edit surface: modal, action chờ, save/discard/continue, registry adapter và chống chạy trùng; form lõi/FBM không còn dispatch qua guard riêng.
...
- [x] PopupList phân biệt popup body và popup viewport: popup body nằm trong hệ tọa độ cuộn của `#sidebar-body`, bị `overflow` che cùng trigger và không bị kẹp vào mép khi trigger rời vùng nhìn thấy; menu header/search vẫn neo theo viewport.
...
- [x] `Đăng nhập thử` và `Kiểm tra liên kết` chạy được ở cả chế độ sửa/xem; sửa dùng giá trị trong form, xem dùng credential/binding đã lưu; test offline `1681/1681`, `fbmGetLoginConfig --push` trả `OK` ở GAS DEV revision `@447`.
- [x] Gộp `Liên kết tài khoản` và `Đăng nhập tự động` thành một card `Kết nối tài khoản FBM` với hai vùng con, một edit surface và một cổng lưu nguyên tử; credential tùy chọn, đổi identity không có credential mới sẽ vô hiệu credential cũ và auto-login; `Đồng bộ nền` vẫn ở `Cài đặt phiên` và card chỉ điều hướng tới đó; test offline `1723/1723`.
- [x] Sửa lỗi ô nhận diện mất focus sau mỗi ký tự: không dựng lại vùng form khi phát `input`, chỉ cập nhật draft và xóa trạng thái lỗi tại đúng dòng; test hồi quy offline `1778/1778`.
