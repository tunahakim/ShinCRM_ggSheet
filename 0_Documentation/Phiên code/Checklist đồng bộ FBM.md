...
| Slice 8 — UI + log + probe | `e243432`, `3e60e88`, `e4c92c1`, `52c3fa9`, `81c4c58` | `1196/1196` | `@216` (`fbmGetMasterSwitch` trả `OK`, `enabled: true`) | UI shell bốn màn hình, menu dọc phù hợp Sidebar hẹp, pill `ON/OFF`, cụm phải `[ON/OFF] [☰]`, pipeline chỉ hiện khi phiên chạy, tab kết quả và phân trang đã hoàn tất; bố cục động dùng Block/schema và renderer chung; chưa nghiệm thu probe trên Sidebar | Cần chủ dự án đóng/mở lại Sidebar để tải deployment mới và kiểm tra hiển thị thực tế |
