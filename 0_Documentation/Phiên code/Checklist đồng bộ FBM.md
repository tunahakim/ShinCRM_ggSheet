...
Ghi chú triển khai sau Slice 8: heartbeat nền kiểm tra relay/công tắc/phiên trước khi gửi FBM; deployment GAS cũ hoặc phiên hết hạn sẽ không làm alarm tiếp tục bắn request FBM. Executor ưu tiên cookie payload hiện có trên tab thay cho cookie cũ từ GAS. Test offline đạt `1232/1232`; GAS DEV đã push revision `@233`, probe trả `RELAY_PROBE_OK` và `backgroundEnabled: false`.
