/**
 * Ca kiểm cho `server/sheet/ConfigRead.js`: bốn khối còn lại của sheet `Config`.
 *
...
function chay(so) {
  section('Bốn khối Config — khóa trùng thì chặn, còn khối sắp xếp thì thứ tự là nghĩa');
  ...
  check(so, 'ba khối đọc theo kiểu tra khóa, khai bằng cặp mã cột',
    Object.keys(hop.CONFIG_READ_BLOCKS).sort(), ['counters', 'defaults', 'sheetSchema']);
  ...
  check(so, 'sheet mới dựng đọc ra ba bảng rỗng và một danh sách rỗng',
    [rong.sheetSchema, rong.defaults, rong.counters, rong.sort], [{}, {}, {}, []]);
  ...
  ghiO(nen, 'Config', hangDau + 1, '@CFG_BO_DEM_LOAI', 'customer');
  ghiO(nen, 'Config', hangDau + 1, '@CFG_BO_DEM_GIA_TRI', 17);
  ...
  check(so, 'ô số ở khối bộ đếm ra chuỗi — cửa cấp mã tự đổi sang số, đây chỉ là đường đọc',
    doc.counters, { customer: '17' });
  ...
  // Trùng ở một khối KHÔNG được làm chết cả sheet trong im lặng: khối khác vẫn phải đọc được nếu đọc riêng.
  check(so, 'khối bộ đếm của cùng sheet đó vẫn đọc riêng được, dù khối ngầm định đang trùng',
    trung.hop.configReadBlock(trung.hop.CONFIG_READ_BLOCKS.counters, trung.hop.readColumnMap('Config'),
      trung.hop.sheetGridReadBlock(trung.sheet('Config'), hangDau, 10, 10)), {});
