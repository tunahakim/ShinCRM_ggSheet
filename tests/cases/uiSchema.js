...
function chay(so) {
  ...
  check(so, 'hàng header của màn view dựng thành sáu Icon, mục "Khác" mang menu chứ không mang action',
    [daDung.view.header.map((n) => n.role).join(','), daDung.view.header[3].action, daDung.view.header[3].menu.length],
    ['icon,icon,icon,icon,icon,icon', null, 2]);
