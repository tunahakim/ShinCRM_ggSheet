...
async function chay(so) {
  ...
  heartbeat.FbmSync.stateStart('', 'idle', 0);
   const missingSessionInitially = heartbeat.fbmSyncHeartbeatRequest({ source: 'alarm' });
   check(so, 'GAS chuyen state sang cho dang nhap khi thieu session', heartbeat.FbmSync.stateRead().phase, 'paused');
  check(so, 'GAS không cấp heartbeat khi chưa biết phiên và chưa có cookie', [missingSessionInitially.ok, missingSessionInitially.code, missingSessionInitially.request], [false, 'AUTO_LOGIN_NOT_CONFIGURED', null]);
  heartbeat.FbmSync.statePatch({ session: { cookie: 'userFHN_CRM_App', expired: false } });
  const heartbeatRequest = heartbeat.fbmSyncHeartbeatRequest({ source: 'alarm' });
  check(so, 'GAS cap envelope heartbeat Customer truoc khi Extension cham FBM', [heartbeatRequest.ok, heartbeatRequest.code, heartbeatRequest.request.meta.kind, heartbeatRequest.request.body.type, heartbeatRequest.request.body.count, heartbeatRequest.request.body.sortExpression], [true, 'HEARTBEAT_REQUEST_READY', 'heartbeat', 0, 1, null]);
  ...
  heartbeat.fbmSyncHeartbeatRequest({ source: 'alarm' });
  ...
  check(so, 'GAS khong cap heartbeat khi phien da het han', [blockedHeartbeatRequest.ok, blockedHeartbeatRequest.code, blockedHeartbeatRequest.request], [false, 'AUTO_LOGIN_NOT_CONFIGURED', null]);
  heartbeat.FbmSync.statePatch({ session: { expired: false } });
  heartbeat.FbmSync.statePatch({ session: { cookie: '', expired: false, lastHeartbeatAt: 0 } });
   const missingCookieRequest = heartbeat.fbmSyncHeartbeatRequest({ source: 'alarm' });
   check(so, 'GAS giu state cho dang nhap khi cookie rong', heartbeat.FbmSync.stateRead().phase, 'paused');
  check(so, 'GAS khong lap heartbeat khi chua lay duoc cookie sau lan dau', [missingCookieRequest.ok, missingCookieRequest.code, missingCookieRequest.request], [false, 'AUTO_LOGIN_NOT_CONFIGURED', null]);
  heartbeat.FbmSync.statePatch({ session: { customerTotal: 2, cookie: 'userFHN_CRM_App', expired: false }, scheduledScan: 'customer', phase: 'idle', runId: '' });
  ...
  const emptyHeartbeat = heartbeat.fbmSyncHeartbeat({ ok: true, status: 200, body: '{"d":{}}' });
  ...
  cancelledState.runId = 'cancel-run'; cancelledState.phase = 'pull_customer'; cancelledState.cursor = { kind: 'customer_grid', pageIndex: 1 }; cancelledState.activeRequestId = 'cancel-request';
  heartbeat.FbmSync.stateWrite(cancelledState);
  heartbeat.fbmSyncCancel();
  const staleAfterCancel = heartbeat.FbmSync.continue({ d: { Rows: [] } });
  check(so, 'response đến sau cancel bị bỏ qua, không chạy tiếp cursor', [staleAfterCancel.ok, staleAfterCancel.code], [false, 'STALE_RESPONSE']);
}
